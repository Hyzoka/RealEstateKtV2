...
import com.ocr.realestatektv2.BuildConfig
...
fun requestSkiResort(
    ...
    onSuccess: (skiResorts: List<Estate>) -> Unit,
    onError: (error: String) -> Unit) {

    service.getSkiResorts().enqueue(
            object : retrofit2.Callback<List<Estate>> {
                override fun onFailure(call: retrofit2.Call<List<Estate>>?, t: Throwable) {
                    Log.e(TAG, "fail to get data", t)
                    onError(t.message ?: "unknown error")
                }

                override fun onResponse(
                    call: retrofit2.Call<List<Estate>>?,
                    response: retrofit2.Response<List<Estate>>
                ) {
...
interface SkiResortListService{
    ...
    fun getSkiResorts(): retrofit2.Call<List<Estate>>
